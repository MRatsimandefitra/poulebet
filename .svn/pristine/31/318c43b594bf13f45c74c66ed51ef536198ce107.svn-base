...
class MatchController extends Controller
{
   ...
   "items":{
      ...
      "c3bee298213b5e15c633f032a37c2b48":{
        ...
        $totalMatch = $data['total'];
        $first = $data['first'];
        $last = $data['last'];
        $timestamp = $data['timestamp_created'];
        $date = date('d-m-Y', $timestamp);
        $items = $data['items'];

        $dateMatchSearch = $request->get('date_match');
        if (!$dateMatchSearch) {

        }
        /*  $arrayMatch = array();
          $arrayMatchFinished = array();
          $arrayMatchStart = array();

          if(is_array($items)){
              foreach($items as $vItems){
                  if($vItems['status']=='start'){
                      $arrayMatchStart = $vItems;
                  }
                  if($vItems['status'] == 'finished'){
                      $arrayMatchFinished = $vItems;
                  }
